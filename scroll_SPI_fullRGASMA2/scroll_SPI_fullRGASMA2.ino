...
#define __TIMER1_CNT 0x0000120// 32 levels --> 0x0130; 38 --> 0x0157 (flicker)
#include <io.h>
#include <interrupt.h>
...
byte lettre[1][1];
int taille[27]={
  2,4,4,4,4,4,4,5,4,2,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4};

byte tab[8]={
  B11111111,B00000000,B11110000,B00001111,B11100011,B1110000,B00000011,B11011100};
byte tab2[8]={
  B11111111,B00000000,B11110000,B00001111,B11100011,B1110000,B00000011,B11011100};
void cls()
{
...
void loop() {
  ...
  while(1){
    ...
    for(i=0;i<=7;i++)
    {
      for( j=0;j<=7;j++)
      {
        screeng[j][i]=(j+counter%8)%8;//__max_brightness*abs(cos((float)j/7*3.14/2+(float)i/7*3.14/2+(float)counter/8));
        // Serial.println(cos(3.14*(float)i/7));
...
ISR(0xF) { /* Framebuffer interrupt routine */
  TCNT1 =  __TIMER1_MAX - __TIMER1_CNT;
  byte cycle;
