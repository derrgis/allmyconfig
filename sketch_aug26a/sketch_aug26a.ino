...
int ledPin = 12; //On board LED

#define STRIP_LENGTH 2 //32 LEDs on this strip
...
void addRandom(long int k) {
  ...
  long int new_color=0;
  long int new_color2=0;
   k = k % (3*256);
    if (k>=0 && k< 256)
    {
      //y=255-x
           new_color= k << 16 | ( (255-k)<<8) | 0 ;
            new_color2= 255-k << 16 | ( (k)<<8) | 0 ;
    ...
    if(k>=256 && k< (256*2))
    {
      ...
      new_color= (255-inside) << 16 | ( 0<<8) | inside ;
      new_color2= 0 << 16 | ( (255-inside)<<8) | inside ;
    ...
    if(k>=(256*2))
    {
      inside=k-256-256;
       ...
       new_color= 0 << 16 | ( inside<<8) | (255-inside) ;
       new_color2= inside << 16 | ( 0<<8) | (255-inside) ;
    }
     strip_colors[0] = new_color;
     strip_colors[1] = new_color2;
