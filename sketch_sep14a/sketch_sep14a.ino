...
void loop() {
    ...
    { 
        if(isBlynkConnected)
        {
          Serial.println("WIFI connection lost stopping Blynk..");
          Blynk.disconnect();
          isBlynkConnected=false;
        }
...
void printDateTime(const RtcDateTime& dt)
{
  ...
  long nowt=dt.Hour()*60+dt.Minute();
  long startday=6*60+30;
  long startnight=22*60+30;
   ...
   if(nowt>=startday && nowt <startnight)
       {        
        bright=70;
