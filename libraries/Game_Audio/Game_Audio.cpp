...
{    
  ...
  if(GameAudioGlobalObject->CurrentWav!=NULL and GameAudioGlobalObject->CurrentWav->Completed==false)
  {
    //if(GameAudioGlobalObject->CurrentWav->Completed==false)
    //{
        unsigned char t=GameAudioGlobalObject->CurrentWav->Data[GameAudioGlobalObject->CurrentWav->DataIdx];
        if(t>127)
        {
            t=127+(t-127)/(256/GameAudioGlobalObject->Volume);
        }
        else
        {
            t=127-(127-t)/(256/GameAudioGlobalObject->Volume);
        }
        dacWrite(GameAudioGlobalObject->DacPin,t);//GameAudioGlobalObject->CurrentWav->Data[GameAudioGlobalObject->CurrentWav->DataIdx]);             												// write out the data
