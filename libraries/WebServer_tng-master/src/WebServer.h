...
#define STREAMFILE_BUFSIZE 2*1460
  ...
  if (buf == NULL) {
    Serial.printf("streamFile malloc failed");
    return 0;
  }
    Serial.printf("streamFile malloc OK");
  size_t totalBytesOut = 0;
  while (client().connected() && (file.available() > 0)) {
      //Serial.printf("in here\n");
    int bytesOut;
    int bytesIn = file.read(buf, STREAMFILE_BUFSIZE);
      //Serial.printf("char lus:%d file size%d\n %d",bytesIn,file.size(),STREAMFILE_BUFSIZE);
    if (bytesIn <= 0) break;
    while (1) {
      bytesOut = 0;
      if (!client().connected()) break;
      bytesOut = client().write(buf, bytesIn);
        //Serial.printf("on ecrit\n");
