...
void i2s_parallel_setup(volatile i2s_dev_t *dev, const i2s_parallel_config_t *cfg) {
    ...
    //pinMode(27,OUTPUT);
    //gpio_matrix_out(27, I2S1O_BCK_OUT_IDX, false, false);
   // gpio_setup_out(27, I2S1O_BCK_OUT_IDX);
    gpio_matrix_out(27, I2S0O_BCK_OUT_IDX, false, false);
    //Initialize I2S dev
    fifo_reset(dev);
    dma_reset(dev);
    ...
    dev->sample_rate_conf.rx_bck_div_num=40000000L/cfg->clkspeed_hz;  //Divider. 40MHz/x
    dev->sample_rate_conf.tx_bck_div_num=40000000L/cfg->clkspeed_hz;
...
void i2s_parallel_start(volatile i2s_dev_t *dev) {
    ...
    dev->out_link.addr=((uint32_t)(&i2s_state[i2snum(dev)]->dmadesc_a[0].buf));
    vTaskDelay(5);
